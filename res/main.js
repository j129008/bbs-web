(function () {
    ...
    var fillList = function () {
        ...
        articleID = articleID.add(baseArticle);
        $('#list').append(listStr);
        $('#base').append(baseStr);
    ...
    var fillContent = function () {
        ...
        while (contentStr.search('GAMEROOTURL') != -1) {
            contentStr = contentStr.replace('GAMEROOTURL', listUrl);
        }
        /*
    ...
    var gotoArticle = function () {
        ...
        if (!data[articleID[select]].readed) {
            var listUrl = window.location.href+'/Articles/';
            listUrl = listUrl.replace('/index.html', '');
            $.ajax({
    ...
    $(function () {
        adjustHead();
        var listUrl = window.location.href+'/Articles/list.txt';
        listUrl = listUrl.replace('/index.html', '');
        $.ajax({
            'url':listUrl
        }).done(function (msg) {
